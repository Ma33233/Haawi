...
class SearchBarViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        collectionViewSearch.register(UINib(nibName: "CustomCollectionViewCell", bundle: nil), forCellWithReuseIdentifier: "sessionCell")
        ProgressHUD.show()
    ...
    func fetchSessionsData(){
        DatabaseManager.shared.fireStore.collection("Sessions").whereField("isClosed", isEqualTo: false).whereField("userID", isNotEqualTo: DatabaseManager.shared.authRef.currentUser!.uid) .addSnapshotListener{ query, error in
            guard error == nil else{
                            ...
                            if let sesstionLat = doc.get("lat") as? Double{
                                if let sesstionLong =  doc.get("long") as? Double{
                                    ...
                                    if sesstionDistance < 10{
                                        if let userName = result?.get("userName") as? String{
                                        let sesstion = Session(userPicture: data, userName: userName, title: doc.get("title")as! String, description: result?.get("description") as! String?, category: doc.get("category") as! String?, userID: doc.get("userID") as! String, sesstionLocation: sesstionDistance, lat: sesstionLat, long: sesstionLong)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        guard segue.identifier == "showMoreInfo" else{
...
extension SearchBarViewController: UISearchBarDelegate{
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        ...
        filteredArr = sesstionsArr.filter{$0.title.lowercased().contains(searchText.lowercased())}
        if searchText.isEmpty{
            filteredArr = sesstionsArr}
...
extension SearchBarViewController: UICollectionViewDelegate{
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        performSegue(withIdentifier: "showMoreInfo", sender: indexPath)
    }
}
